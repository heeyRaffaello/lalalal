...
private:
    ...
    TELEM *mass; // память для представления битового поля
    int  Size; // к-во эл-тов Мем для представления бит.поля
    
    // методы реализации
    int   GetIndex(const int BitPos) const; // индекс в pМем для бита n       (#О2)
    TELEM GetMask (const int BitPos) const; // битовая маска для бита n       (#О3)
public:
...
#endif
#endif /* defined(___________________) */
